...
if($_POST['txtUsername'] != ""){			
	...
	$adldap -> connect();
	if($adldap -> authenticate($_POST['txtUsername'],$_POST['txtPassword'])){
		$userinfo = $adldap->user_info($_POST['txtUsername'], array("displayname","description","telephonenumber","givenname","sn","mobile","title","department","company","mail"), $isGUID);
		$_SESSION["login"] = true;
		...
		if($adldap -> authenticate($operator_adUser,$operator_adPassword,true)){
			$userinfo = $adldap->user_info($_POST["txtUsername"], array("displayname","useraccountcontrol","lockoutTime"), $isGUID);
				if($userinfo){
