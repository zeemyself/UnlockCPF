...
class captchaZDR {
  ...
  function LoadPNG(){  
     ...
     /* if (!$im) { 
           $im  = imagecreatetruecolor(220, 40); /* Create a blank image */
        /*   $bgc = imagecolorallocate($im, 255, 255, 255);
           $tc  = imagecolorallocate($im, 0, 0, 0);
           imagefilledrectangle($im, 0, 0, 220, 40, $bgc);
...
imagesavealpha($image, true);
  ...
  function task_string(){
        ...
        $string_a = array("A","B","C","D","E","F","G","H","K",
        ...
        for($i=0;$i<5;$i++)
        {
            ...
            $angle      = rand(-15,15);
            // Add the text
            $width_pos  = rand(20,20);
            $width      = $width  + $width_pos;
//            $height     = rand(35,75);
            $height     = rand(35,35);
            $temp       = $string_a[rand(0,25)];
            $this->UserString .= $temp;
            imagettftext($image, 15, $angle, $width, $height, $colour, $font, $temp);
            $width    = $width;
            $height   = $height;
            imagettftext($image, 15, $angle, $width, $height, $colour, $font, $temp);
